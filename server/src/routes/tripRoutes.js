...
router.get('/self', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const tripObjectIds = trips.map((trip) => trip._id);
    ...
    return res.status(200).json({
      trips: trips.map((trip) => {
        ...
        return {
          ...
          participantIds,
          pendingRequestCount: pendingCountByTripId[id] ?? 0,
...
router.get('/:tripId/requests', requireAuth, async (req, res) => {
  ...
  const status = typeof requestedStatus === 'undefined' ? 'pending' : isRequestStatus(requestedStatus) ? requestedStatus : null;
  if (!status) {
...
router.post('/:tripId/join', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const hostObjectId = new mongoose.Types.ObjectId(String(trip.organizerId));

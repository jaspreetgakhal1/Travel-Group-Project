...
const loadTripContext = async (tripId, requesterId) => {
  if (!mongoose.isValidObjectId(tripId) || !mongoose.isValidObjectId(requesterId)) {
  ...
  const orderedMemberIds = Array.from(new Set([String(trip.organizerId), ...((Array.isArray(trip.participants) ? trip.participants : []).map((participantId) => String(participantId)))]));
...
router.get('/trips/:tripId', requireAuth, async (req, res) => {
  ...
  try {
    const summary = await buildExpenseSummary(req.params.tripId, requesterId);
    if (summary.error) {
...
router.post('/split', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const payerId = requesterId;
    const settlements = memberIds
      .filter((memberId) => memberId !== payerId)
      .map((memberId) => ({
        userId: new mongoose.Types.ObjectId(memberId),
        owesToUserId: new mongoose.Types.ObjectId(payerId),
        amount: splitAmount,
      }));

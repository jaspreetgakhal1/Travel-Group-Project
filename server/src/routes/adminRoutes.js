...
import { ACTIVE_TRIP_STATUS, CANCELLED_TRIP_STATUS, CANCELLED_TRIP_STATUS_VALUES, COMPLETED_TRIP_STATUS, COMPLETED_TRIP_STATUS_VALUES, normalizeTripRecordStatus, UPCOMING_TRIP_STATUS, } from '../utils/tripRecordStatus.js';
...
const parseQueryDateBoundary = (value, boundary) => {
...
const buildTripDateMatch = (fromDate, toDate) => {
    if (fromDate && toDate) {
        return {
...
const buildUserPostMatchConditions = (user) => {
    ...
    const postMatchConditions = [{ author: user._id }];
    if (normalizedEmail) {
...
const syncUserPostVerification = async (user, isVerified) => {
...
const createVerificationNotification = async ({ userId, title, message, type, rejectionReason = null, }) => {
...
const normalizeVerificationStatus = (value) => value === 'verified' ? 'verified' : value === 'rejected' ? 'rejected' : 'pending';
const serializeAdminUser = (user) => ({
    ...
    verificationDocumentMimeType: typeof user.verificationDocumentMimeType === 'string' ? user.verificationDocumentMimeType : null,
...
const serializeDeletedAdminUser = (user) => ({
    id: String(user._id),
...
const serializeAdminTrip = (trip) => {
    const normalizedStatus = normalizeTripRecordStatus(typeof trip.status === 'string' ? trip.status : null, {
...
const ensureNotSelfTarget = (requestUserId, targetUserId) => {
    if (!requestUserId) {
...
const deleteUserCascade = async (userId) => {
    const organizedTrips = await Trip.find({ organizerId: userId }).select('_id').lean();
...
router.get('/users', async (req, res) => {
    ...
    const verificationStatusFilter = requestedStatus === 'pending' ||
    ...
    try {
        if (verificationStatusFilter === 'deleted') {
            const deletedUsers = await DeletedUser.find({})
                ...
                .select('_id originalUserId userId firstName lastName email mobileNumber profileImageDataUrl isBlocked blockedAt blockedReason verificationStatus verificationDocumentName verificationDocumentMimeType verificationUploadedAt rejectionReason createdAt deletedAt')
                .lean();
            return res.status(200).json({
                users: deletedUsers.map(serializeDeletedAdminUser),
            });
        }
        const userFilter = verificationStatusFilter === 'blocked'
            ? { isBlocked: true }
        ...
        const users = await User.find(userFilter)
            ...
            .select('_id userId firstName lastName email mobileNumber profileImageDataUrl isBlocked blockedAt blockedReason verificationStatus verificationDocumentUrl verificationDocumentName verificationDocumentMimeType verificationUploadedAt rejectionReason createdAt')
            .lean();
        return res.status(200).json({
...
router.get('/pending-users', async (req, res) => {
    ...
    try {
        ...
        const pendingUsers = await pendingUsersQuery.lean();
        return res.status(200).json({
...
router.post('/verify-user/:id', async (req, res) => {
    ...
    try {
        const user = await User.findById(userId).select('_id email userId firstName lastName verificationDocumentUrl verificationStatus');
        if (!user) {
...
router.post('/block-user/:id', async (req, res) => {
    ...
    const reason = typeof rawReason === 'string' && rawReason.trim() ? rawReason.trim() : 'Blocked by administrator.';
...
router.delete('/user/:id', async (req, res) => {
    ...
    try {
        const user = await User.findById(targetUserId).select('_id userId firstName lastName email mobileNumber profileImageDataUrl isBlocked blockedAt blockedReason verificationStatus verificationDocumentName verificationDocumentMimeType verificationUploadedAt rejectionReason createdAt');
        if (!user) {
            return res.status(404).json({ message: 'User account not found.' });
        }
...
router.get('/stats', async (req, res) => {
    ...
    try {
        ...
        const [totalUsers, totalVerifiedUsers, totalPendingUsers, totalBlockedUsers, totalDeletedUsers, tripStatsResult] = await Promise.all([
            User.countDocuments({}),
            User.countDocuments({
            ...
            Trip.aggregate(tripAggregationPipeline),
        ]);
        const tripStats = tripStatsResult[0] ?? {};
        const tripCounts = new Map((tripStats.counts ?? []).map((entry) => [entry._id, typeof entry.count === 'number' ? entry.count : 0]));
        ...
        const grossTripTotal = typeof totals.grossTripTotal === 'number' ? Number(totals.grossTripTotal.toFixed(2)) : 0;

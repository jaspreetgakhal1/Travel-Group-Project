...
const FALLBACK_SUGGESTION_BLUEPRINTS = [
  {
    ...
    buildReason: (input: {
      ...
      `A relaxed introduction to ${input.destination} that matches a ${input.collectiveMood.toLowerCase()} group vibe and keeps the day easy to coordinate.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `This stop leans into ${input.interest.toLowerCase()} while giving the group something visual, social, and easy to enjoy together.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `It fits the group's ${input.food.toLowerCase()} preference and creates a flexible meal stop that still feels memorable.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `A quieter change of pace that works well for groups asking for ${input.crowds.toLowerCase()} experiences without losing the local feel.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `It gives the group a simple shared highlight near the end of the day and keeps the plan practical for mixed travel styles.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `This leans into ${input.interest.toLowerCase()} with a structured cultural stop that still fits the group's ${input.budget.toLowerCase()} spending style.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `It matches ${input.interest.toLowerCase()} and gives the group a ${input.collectiveMood.toLowerCase()} outdoor reset without making the day expensive.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `This works for ${input.interest.toLowerCase()} travelers and keeps food flexible for a ${input.food.toLowerCase()} stop along the way.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `It suits a ${input.collectiveMood.toLowerCase()} group that wants ${input.food.toLowerCase()} and a more social, easy-to-coordinate stop.`,
  },
  {
    ...
    buildReason: (input: {
      ...
      `This is a stronger fit for ${input.food.toLowerCase()} and ${input.budget.toLowerCase()} plans where the group wants one polished shared highlight.`,
  },
] as const;

type CuratedFallbackSuggestion = {
  name: string;
  imageKeyword: string;
  baseCost: number;
  vibe: number;
...
type CuratedDestinationFallback = {
  aliases: string[];
  suggestions: CuratedFallbackSuggestion[];
};

const CURATED_DESTINATION_FALLBACKS: CuratedDestinationFallback[] = [
  {
    ...
    suggestions: [
      {
        ...
        vibe: 92,
        buildReason: (input) =>
          `This Toronto favorite matches a ${input.collectiveMood.toLowerCase()} mood with cobblestone lanes, galleries, cafes, and an easy group walking pace.`,
      },
      {
        ...
        vibe: 88,
        buildReason: (input) =>
          `It gives the group a high-impact Toronto landmark stop with skyline views and a crowd-pleasing indoor experience that works well for ${input.interest.toLowerCase()} energy.`,
      },
      {
        ...
        vibe: 91,
        buildReason: (input) =>
          `Kensington fits a ${input.food.toLowerCase()} plan with casual local eats, vintage corners, and a social street atmosphere that still feels flexible.`,
      },
      {
        ...
        vibe: 89,
        buildReason: (input) =>
          `The islands are ideal when the group wants ${input.crowds.toLowerCase()} moments, open waterfront views, and a softer break from downtown.`,
      },
      {
        ...
        vibe: 87,
        buildReason: (_input) =>
          'This is one of the easiest Toronto stops for sharing local snacks, picking up quick bites, and keeping the day practical for a group.',
      },
    ],
  },
  {
    ...
    suggestions: [
      {
        ...
        vibe: 91,
        buildReason: (input) =>
          `This is a strong Bali opener for a ${input.collectiveMood.toLowerCase()} group, with iconic scenery, photo moments, and an easy half-day rhythm.`,
      },
      {
        name: 'Uluwatu Temple',
        ...
        vibe: 90,
        buildReason: (input) =>
          `Uluwatu blends cliff views and Balinese culture in a way that fits ${input.interest.toLowerCase()} travelers without feeling overly rushed.`,
      },
      {
        ...
        vibe: 88,
        buildReason: (input) =>
          `This stop works well for groups who want shopping, local design, and a central Bali base that still feels authentic and easy to explore together.`,
      },
      {
        ...
        vibe: 89,
        buildReason: (input) =>
          `Canggu gives the group a casual Bali social scene with sunset energy, cafe access, and enough space to match a ${input.crowds.toLowerCase()} preference.`,
      },
      {
        ...
        vibe: 87,
        buildReason: (_input) =>
          'It adds a meaningful cultural stop to the Bali plan and feels memorable without requiring a complicated full-day schedule.',
      },
    ],
  },
  {
    ...
    suggestions: [
      {
        ...
        vibe: 90,
        buildReason: (input) =>
          `Wat Arun is a beautiful Thailand pick for groups who want culture, river views, and a ${input.collectiveMood.toLowerCase()} pace that still feels iconic.`,
      },
      {
        ...
        vibe: 88,
        buildReason: (input) =>
          `This stop delivers one of Thailand's strongest landmark experiences and fits ${input.interest.toLowerCase()} travelers who want something unmistakably local.`,
      },
      {
        ...
        vibe: 91,
        buildReason: (input) =>
          `Chatuchak is perfect for a ${input.food.toLowerCase()} and market-heavy day, with low-pressure browsing, snacks, and group-friendly variety.`,
      },
      {
        ...
        vibe: 87,
        buildReason: (input) =>
          `Chiang Mai offers a calmer Thailand option for groups looking for ${input.crowds.toLowerCase()} experiences without losing the sense of place.`,
      },
      {
        ...
        vibe: 89,
        buildReason: (_input) =>
          'This adds Thailand scenery, beach time, and a memorable visual payoff that feels worth sharing as a group highlight.',
      },
    ],
  },
];
...
const optimizeUnsplashImageUrl = (value: string): string => {
...
const normalizeBudgetLabel = (value: string): string => value.trim().toLowerCase();

const normalizeDestinationLookup = (value: string): string =>
  value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9\s]/g, ' ')
...
const findCuratedDestinationFallback = (destination: string): CuratedDestinationFallback | null => {
  ...
  return (
    CURATED_DESTINATION_FALLBACKS.find((entry) =>
      entry.aliases.some((alias) => normalizedDestination.includes(alias) || alias.includes(normalizedDestination)),
    ) ?? null
  );
};

const hasOnlyGenericSuggestionNames = (suggestions: GeneratedTripSuggestion[]): boolean =>
  suggestions.length > 0 &&
  suggestions.every((suggestion) =>
    GENERIC_SUGGESTION_NAME_PATTERNS.some((pattern) => pattern.test(suggestion.name)),
  );
...
const scoreSuggestionForPreferences = (
...
const parseGeminiApiError = async (response: Response): Promise<GeminiApiError> => {
  const errorText = await response.text();
  const error = new Error(errorText || 'Gemini could not generate suggestions right now.') as GeminiApiError;
  error.statusCode = response.status;
  return error;
};

const isFallbackWorthyGeminiError = (error: unknown): boolean => {
  if (!(error instanceof Error)) {
    return false;
  }

  const statusCode = typeof (error as GeminiApiError).statusCode === 'number' ? (error as GeminiApiError).statusCode : null;

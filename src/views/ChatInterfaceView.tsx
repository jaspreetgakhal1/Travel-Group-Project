...
const ChatInterfaceView: React.FC<ChatInterfaceViewProps> = ({ messages }) => {
  return (
    <section className="rounded-card bg-white/95 p-5 shadow-lg ring-1 ring-primary/10 backdrop-blur-sm">
      ...
      <form className="mt-4 flex gap-2 border-t border-primary/10 pt-4">
        <input
          ...
          className="w-full rounded-card border border-primary/20 bg-background/80 px-4 py-2.5 text-sm text-primary outline-none ring-accent/35 transition focus:ring-2"
        />
        <button
          type="submit"
          className="rounded-card bg-primary px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90"
        >
          Send
        </button>
      </form>

...
const isMongoObjectId = (value: string): boolean => OBJECT_ID_PATTERN.test(value);

const TripCardView: React.FC<TripCardViewProps> = ({
  trip,
  matchScore,
  currentUserId = null,
  ...
  useEffect(() => {
    ...
    const resolveChatAccess = async () => {
      if (!currentUserId || !resolvedAuthToken || !isMongoObjectId(trip.id)) {
        if (isActive) {
      ...
      const isAllowed = await checkTripChatAccess(trip.id, resolvedAuthToken);
      if (isActive) {
        setHasChatAccess(isAllowed);
      }
    };

    void resolveChatAccess();

    return () => {
      isActive = false;
    };
  }, [currentUserId, resolvedAuthToken, trip.id]);

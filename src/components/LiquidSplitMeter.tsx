...
const clamp = (value: number) => Math.min(0.96, Math.max(0.08, value));

const LiquidSplitMeter: React.FC<LiquidSplitMeterProps> = ({ fillRatio, label, valueLabel }) => {
  const prefersReducedMotion = useReducedMotion();
  const clipPathId = useId();
  const gradientId = useId();
  const normalizedFill = clamp(fillRatio);
  const liquidY = 12 + (1 - normalizedFill) * 144;
  ...
  return (
    <div className="flex flex-col items-center gap-3">
        ...
        <svg viewBox="0 0 138 216" className="absolute inset-0 h-full w-full" fill="none" aria-hidden="true">
          <defs>
            ...
            <linearGradient id={gradientId} x1="0%" y1="100%" x2="0%" y2="0%">
              <stop offset="0%" stopColor="rgba(129,178,154,0.95)" />
              <stop offset="100%" stopColor="rgba(244,241,222,0.88)" />
            </linearGradient>
          </defs>
      ...
      <p className="max-w-[140px] text-center text-[11px] font-medium leading-5 text-primary/50">
        The wave refills live as you adjust the expense and who is joining the split.
      </p>
    </div>
  );
};
